...
def forward_propagation(X, parameters):
    ...
    with tf.device('/gpu:0'):
        ...
        #print(p2) UNCOMMENT TO VIEW STRUCTURE
        P2 = tf.contrib.layers.fully_connected(P2, 128)
        P2 = tf.nn.relu(P2)
        P2 = tf.nn.dropout(P2, 0.4)
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.Session() as sess:
            ...
            for minibatch in minibatches:
                    ...
                    # Run the session to execute the optimizer and the cost, the feedict should contain a minibatch for (X,Y).
                    train_accuracy_cmp.extend( correct_prediction.eval(feed_dict = {X: minibatch_X, Y: minibatch_Y}))
                    #print("train batch: ",train_accuracy)
                    
            
            
            
            
            train_accuracy = sum(train_accuracy_cmp)/len(train_accuracy_cmp)        
            ...
            #print(sess.run(predict_op, feed_dict = {X:X_test[0].reshape((1,28,28,1))}))
...
X_train, y_train = load_mnist('F:\machine learning\code\mnist1', kind='train')
X_test, y_test = load_mnist('F:\machine learning\code\mnist1', kind='t10k')
